...
import os
from actor_recognition_module.face_rec.MLModel import MLModel
...
class FaceRecModel(MLModel):
    ...
    def __linear_search(self, query_encodings):
        ...
        for encoding in query_encodings:
            matches = face_recognition.compare_faces(self.known_encodings, encoding, tolerance=config.TOLERANCE)
            name = config.ID_UNKNOWN
    ...
    def save(self, path):
        ...
        """
        encoding_structure = config.ENC_LIST

        data =  {   config.KNOWN_ENCODINGS: self.known_encodings,
                    config.KNOWN_NAMES: self.known_names,
                    config.ENCODING_STRUCTURE: encoding_structure }
    ...
    def load_from_file(self, path):
        ...
        self.known_encodings, self.known_names = data[config.KNOWN_ENCODINGS], data[config.KNOWN_NAMES]